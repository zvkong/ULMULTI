...
stopifnot(setequal(sort(unique(pums$PUMA)), area_levels))
...
pred_idx <- match(pcells$PUMA, rownames(basis_mat))
stopifnot(!anyNA(pred_idx))
predPsi <- basis_mat[pred_idx, , drop = FALSE]
...
for (k in seq_len(n_sim)) {
  ...
  mod_idx <- match(samp$PUMA, rownames(basis_mat))
  stopifnot(!anyNA(mod_idx))
  modPsi <- basis_mat[mod_idx, , drop = FALSE]
  modY <- samp$INCO
  modZ <- samp$POV
...
saveRDS(
  ...
  "empirical_indicator_nongroup_basis_results.rds"
)

ggplot2::ggsave("empirical_indicator_nongroup_basis_plot.png", final_plot, width = 12, height = 6, dpi = 300)
save.image("empirical_indicator_nongroup_basis_results.RData")
