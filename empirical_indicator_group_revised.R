...
for (k in seq_len(n_sim)) {
  ...
  group_id <- interaction(samp$PUMA, samp$SEX, samp$BACH, drop = TRUE, lex.order = TRUE)
  grp <- collapse_indicator_binomial(
    X = modX,
    Y = modZ,
    S = modPsi,
    wgt = modwgt,
...
saveRDS(
  ...
  "empirical_indicator_group_results.rds"
)

ggplot2::ggsave("empirical_indicator_group_plot.png", final_plot, width = 12, height = 6, dpi = 300)

save.image("empirical_indicator_group_results.RData")
